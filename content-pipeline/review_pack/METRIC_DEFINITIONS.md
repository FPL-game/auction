...
| **Nearest captured opponent (360)** | Distance from a shot to the closest non-teammate position in that event's freeze-frame. | Euclidean distance, `sqrt((x1-x2)^2+(y1-y2)^2)`, using StatsBomb's embedded `shot.freeze_frame` | locally_calculated (from provider-supplied positions) | n/a (direct geometry) |
...
| **Team width (possession)** | How spread out a team is side-to-side while in possession, averaged per phase. | `(team_in_possession_width_start + team_in_possession_width_end) / 2`, averaged across a half's phases | locally_calculated average over a provider_supplied per-phase field | n/a (arithmetic only) |
